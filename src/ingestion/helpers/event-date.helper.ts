...
/**
 * Extracts a Date suitable for chronological comparison from an Event.
 *
 * Attempts to resolve the event's primary datetime (accepting common representations), falls back to `event.createdAt` if present, and uses the current date/time if none is available.
 *
 * @param event - The event to extract the comparable date from.
 * @returns The resolved Date to use for comparisons: the event's datetime when available, otherwise `event.createdAt`, otherwise the current date/time.
 */
 ...
 * @returns `true` if `date1` and `date2` represent the same calendar day, `false` otherwise.
 */
