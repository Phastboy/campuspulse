...
export class EventsReadController {
  ...
  @Get()
  @ApiOperation({ summary: 'List events', description: 'Returns published events ordered by date ASC.' })
  ...
  async findAll(@Query() query: EventQueryDto): Promise<AppApiResponse<unknown>> {
    const result = await this.eventsReadService.findAll({
      fromDate: query.fromDate, toDate: query.toDate,
      type: query.type, limit: query.limit, offset: query.offset,
    });
    return AppApiResponse.ok(result);
  }

  @Get('venue/:venue')
  @ApiOperation({ summary: 'Find events by venue', description: 'Case-insensitive partial match.' })
  @ApiParam({ name: 'venue', example: 'Trust Hall' })
  @ApiResponse({ status: 200 })
  async findByVenue(@Param('venue') venue: string): Promise<AppApiResponse<unknown>> {
