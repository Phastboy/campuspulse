...
import { EventSummary, EventSubmission, SimilarityMatch } from '@application/types';
import { type IEventCandidateReader, EVENT_CANDIDATE_READER } from '@ports/events/event-candidate-reader.port';
...
export class SimilarityEngine implements ISimilarityEngine {
  ...
  async findSimilar(submission: EventSubmission): Promise<SimilarityMatch[]> {
    ...
    const candidates = await this.candidateReader.findCandidatesInWindow(from, to);
    ...
    return scored
      .filter((r): r is SimilarityMatch => r !== null && r.score > this.SIMILARITY_THRESHOLD)
  ...
  private async scoreCandidate(candidate: EventSummary, submission: EventSubmission): Promise<SimilarityMatch | null> {
    try {
      const context: SimilarityContext = { submission, candidate, submissionDate: submission.datetime.date };
      ...
      this.logger.error(`Failed to score candidate ${candidate.id}: ${message}`);
      return null;
    }
  }
}
