...
export class Migration20260318183117 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "users" ("id" uuid not null default uuid_generate_v4(), "google_id" varchar(255) not null, "email" varchar(255) not null, "username" varchar(255) not null, "created_at" timestamptz not null, primary key ("id"));`);
    ...
    this.addSql(`create table "refresh_tokens" ("id" uuid not null default uuid_generate_v4(), "jti" varchar(255) not null, "user_id" uuid not null, "expires_at" timestamptz not null, "created_at" timestamptz not null, primary key ("id"));`);
    this.addSql(`create index "refresh_tokens_jti_index" on "refresh_tokens" ("jti");`);

    this.addSql(`alter table "refresh_tokens" add constraint "refresh_tokens_user_id_foreign" foreign key ("user_id") references "users" ("id");`);
  }
