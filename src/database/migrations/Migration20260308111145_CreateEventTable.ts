...
export class Migration20260308111145_CreateEventTable extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "event" ("id" uuid not null default uuid_generate_v4(), "title" varchar(200) not null, "description" text null, "category" text check ("category" in ('tech', 'academic', 'entertainment', 'sports', 'welfare', 'online', 'other')) not null, "date" timestamptz not null, "venue" varchar(255) not null, "location_tag" text check ("location_tag" in ('on-campus', 'off-campus', 'online')) not null, "pricing_tag" text check ("pricing_tag" in ('free', 'paid', 'conditional')) not null, "pricing_detail" text null, "rsvp_status" text check ("rsvp_status" in ('open-entry', 'register', 'buy-ticket')) not null, "rsvp_link" varchar(255) null, "contact_info" varchar(255) null, "organizer" varchar(255) null, "source" varchar(255) not null, "status" text check ("status" in ('pending', 'live', 'cancelled', 'postponed')) not null default 'pending', "created_at" timestamptz not null, "updated_at" timestamptz not null, constraint "event_pkey" primary key ("id"));`,
    );
    this.addSql(
